...
\titlerunning{SPANG: Template Library for Making SPARQL Queries FAIR}
...
Thsu, applying FAIR principle to SPARQL query should increase the reusability of the queries.
...
According to the FAIR principle, we identified the process to make SPARQL query reusable.
...
%SPARQL queries are written and accumulated. However, those SPARQL queries 
%are not findable, due to the lack of export and import SPARQL queries.
...
%The core technology constituting the Semantic Web is RDF, OWL, and SPARQL. 
%RDF and OWL follows the FAIR principle. However, SPARQL is not FAIR.
...
% \end{itemize}
%Some of the SPARQL libraries are accessible from internet. 
%However, we could consider more general scheme for accessibility. 
%In the FAIR principle, accessibility includes minting URI for each resource.
...
For interoperability, a shared language is necessary.
...
Indeed, it is valuable to use several triplestore implementation through the same programming interface. 
...
%Reusability of SPARQL is not a new topic. 
The ultimate goal of FAIR is to optimise the reuse of resources. 
However, in the specification of SPARQL, reusability is not considered as a central topic. 
...
%If it is a component of Semantic Web comprised of RDF/OWL/SPARQL, it is reasonable to represent it in RDF framework. 
%We propose a way to represent SPARQL in the RDF framework.
...
The use case shown in this section includes the query to the DisGeNET endpoint~\cite{disgenet} shown in Figure~\ref{fig:example-sparql}.
